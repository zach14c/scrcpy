...
static SDL_bool get_serial(libusb_device *device, struct libusb_device_descriptor *desc, unsigned char *data, int length) {
    ...
    if (r) {
        // silently ignore
        LOGD("USB: cannot open device %04x:%04x", desc->idVendor, desc->idProduct);
        return SDL_FALSE;
    }

    if (libusb_get_string_descriptor_ascii(handle, desc->iSerialNumber, data, length) <= 0) {
        // silently ignore
        LOGD("USB: cannot read serial of device %04x:%04x", desc->idVendor, desc->idProduct);
        return SDL_FALSE;
    }
